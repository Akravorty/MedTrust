...
from .conftest import make_label_image_with_qr
...
def test_finalization_event_sent_to_ledger(app_client):
    ...
    app_client.post(f"/intake/batches/{batch_id}/finalize")
...
def test_idempotent_finalize_does_not_duplicate_ledger_event(app_client):
    ...
    app_client.post(f"/intake/batches/{batch_id}/finalize")
    app_client.post(f"/intake/batches/{batch_id}/finalize")  # second call, should be a no-op
